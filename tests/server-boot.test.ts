...
function listToolsViaStdio(entry: string, cwd: string): Promise<string[]> {
  return new Promise((resolve, reject) => {
    const child = spawn('node', [entry], {
      cwd,
      // No env at all: this server needs no credentials, so it must boot and
      // serve tools/list in a bare environment.
      env: { ...process.env, MPAZ_SITE_URL: '', MPAZ_SCHOOL_ID: '' },
...
describe('server boot (built artifacts)', () => {
  it('bundled .mcpb (dist/bundle.js) boots WITHOUT node_modules and lists tools', async () => {
    const dir = mkdtempSync(join(tmpdir(), 'mpaz-mcpb-'));
    try {
      ...
      expect(tools).toContain('mpaz_healthcheck');
    } finally {
